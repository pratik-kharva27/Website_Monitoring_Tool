  ...
  const handleCheckStatus = async (id, url) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:5000/api/websites/check-status",
        { id, url }
      );
      ...
      setWebsites((prevWebsites) =>
        prevWebsites.map((website) =>
          website.id === id
            ? { ...website, status: updatedStatus, lastChecked: new Date() }
            : website
        )
      );
        ...
        <tbody>
          {displayedWebsites.map((website) => (
            <tr
              ...
              style={{
                backgroundColor:
                  website.status === "Down" ? "red" : "transparent",
              }}
            >
              <td>{website.url}</td>
              ...
              <td>
                <button
                  onClick={() => handleCheckStatus(website.id, website.url)}
                  className="btn-all"
                >
                  Check Status
                </button>
